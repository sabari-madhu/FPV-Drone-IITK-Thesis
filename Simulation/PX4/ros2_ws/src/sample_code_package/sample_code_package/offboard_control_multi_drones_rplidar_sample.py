...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        rplidar_topic = f"/{namespace}/rplidar_scan"
        self.get_logger().info(f"Lidar topic : {rplidar_topic}")
    ...
    def print_lidar_sensor_val(self, scan):
        ...
        single_truth = lambda lst: bool(sum(1 if ele else 0 for ele in lst))
        true_hits = list(map(validate_sample, scan.ranges))

        quadrants = {
            "BACK": single_truth(true_hits[0:55] + true_hits[305:360]),
            "LEFT": single_truth(true_hits[35:145]),
            "FRONT": single_truth(true_hits[125:235]),
            "RIGHT": single_truth(true_hits[215:325])
        }

        identified_dirs = [dir for dir, hit in quadrants.items() if hit]

        if identified_dirs:
            self.get_logger().info(f"Obstacle angles : {identified_dirs}")
